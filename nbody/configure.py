...
# (--clean で削除する)。nbody には該当なし。
RUN_ARTIFACTS: List[str] = []

# 実行結果の出力先 (--clean では残し、--allclean のときだけ削除する)
#   log/<file>_run.csv … common/io.hpp の write_log / io_mod.f90 の io_write_log が
#   追記する測定ログ。summary/parse_logs.py の入力なので既定では消さない。
# HDF5 スナップショット出力 (旧 dat/) はコードごと廃止したため存在しない。
OUTPUT_ARTIFACTS: List[str] = ["log"]
...
def detect_actual_machine() -> Optional[str]:
    ...
    環境変数 <APP>_SKIP_HOSTCHECK=1 (このアプリでは NBODY_SKIP_HOSTCHECK=1) でスキップ可能。
...
def generate_job_script(cfg: dict, args, machine: dict, machine_name: str,
    ...
    # 混合精度アプリ (app.yaml に default_fp_low/mid あり) のバイナリ名は
    # <target>.<mode>.<FP_L>.<FP_M> (Makefile / CMake 側の命名と合わせる)。
    fp_low = cfg.get("default_fp_low")
    fp_mid = cfg.get("default_fp_mid")
    if fp_low is not None and fp_mid is not None:
        bin_name = f"{cfg['target_base']}.{mode}.{fp_low}.{fp_mid}{bin_suffix}"
    else:
    ...
    lines += [
        "",
        "# === 実行 ===",
        "#   BENCHMARK_MODE では粒子数を対数的に振って測定する。",
        "#   引数なしで既定値 (num_min=1024, num_max=4194304, num_bin=13,",
        "#   elapse_min=1.0 [s]) が使われる。変更する場合は下のコメントを参照:",
        "#     --num_min N --num_max N --num_bin N --elapse_min SEC --file NAME",
        "#   使用可能なオプション一覧は --help で表示される。",
        f"{bin_rel}",
    ]
...
def generate_cmake(cfg: dict, args, machine_name: str) -> Tuple[Path, str]:
    ...
    lines += [
        ...
        f"",
        # nbody のソースが参照するのは FP_L / FP_M / FP_H。FP は他アプリ (diffusion/fem) と
        # 共通の cache 変数として出しているが、この app のソースからは参照されない。
        f"# Cache variables (nbody は cmake -DFP_L=64 -DFP_M=64 等で上書き。FP はソース未参照)",
        f'set(FP "{cfg["default_fp"]}" CACHE STRING "Floating point bits (未参照; nbody は FP_L/FP_M を使う)")',
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        choices=[32, 64],
        default=None,
        help="浮動小数精度の default 値を指定 (32 or 64)。nbody は混合精度アプリなので "
             "FP_L (低精度部) と FP_M (標準精度部) の両方に反映される (FP_H は 64 固定)。"
             "省略時は app.yaml の default_fp_low / default_fp_mid (どちらも 32)。"
             "混合精度にする場合は cmake -DFP_L=32 -DFP_M=64 のように個別指定する。",
    )
    p.add_argument(
        "--nthreads",
        type=int,
...
def main() -> int:
    ...
    if args.fp is not None:
        ...
        # nbody は混合精度アプリで、ソース (src/common/type.hpp) が見るのは FP_L / FP_M / FP_H
        # だけ (マクロ FP は未参照)。--fp は「低精度部と標準精度部を揃えて切り替える」指定として
        # FP_L / FP_M の両方に反映する。混合 (FP_L != FP_M) は cmake -DFP_L=.. -DFP_M=.. で指定する。
        if cfg.get("default_fp_low") is not None:
            cfg["default_fp_low"] = args.fp
        if cfg.get("default_fp_mid") is not None:
            cfg["default_fp_mid"] = args.fp
    ...
    # nbody は混合精度アプリ: 実際に効くのは FP_L / FP_M / FP_H で、マクロ FP はソース側で未参照
    if (cfg.get("default_fp_low") is not None) and (cfg.get("default_fp_mid") is not None):
        print(f"  precision     : FP_L={cfg['default_fp_low']} / FP_M={cfg['default_fp_mid']} / FP_H=64 (固定)"
              f"  (--fp で FP_L・FP_M を一括変更、cmake -DFP_L=32 -DFP_M=64 で混合精度)")
    else:
    ...
    # 混合精度アプリのバイナリ名は <target>.<mode>.<FP_L>.<FP_M>[.<NTHREADS>] (CMake / Makefile 側と同じ)
    fp_label = ("<FP_L>.<FP_M>"
                if (cfg.get("default_fp_low") is not None and cfg.get("default_fp_mid") is not None)
                else "<FP>")
    print(f"  target binary : {cfg.get('target_base', '?')}.{args.mode}.{fp_label}" +

...
def resolve_build_config(args, app, impls, machine, machine_name) -> dict:
    ...
    # app.yaml の defines_default を追加 (nbody の SIMD_BITS, MEMORY_ALIGNMENT, FP_H 等)
    defaults_block = app.get("defines_default") or {}
    for k, v in defaults_block.items():
        if v == "" or v is None:
            defines.append((k, None))            # 値無し (#define X)
        else:
            defines.append((f"{k}={v}", None))   # リテラル値 (#define X N)

    # FP_L / FP_M (混合精度) — app.yaml に default_fp_low/mid があれば追加
    fp_low = app.get("default_fp_low")
    fp_mid = app.get("default_fp_mid")
    if fp_low is not None:
        defines.append(("FP_L", "FP_L"))
    if fp_mid is not None:
        defines.append(("FP_M", "FP_M"))
...
def generate_cmake(cfg: dict, args, machine_name: str) -> Tuple[Path, str]:
    ...
    # nbody は FP_L.FP_M 表記、それ以外は FP 単一
    ...
    # nbody 用 FP_L / FP_M (app.yaml に default_fp_low/mid があれば追加)
    if cfg.get("default_fp_low") is not None:
        lines.append(f'set(FP_L "{cfg["default_fp_low"]}" CACHE STRING "low precision (FP_L)")')
    if cfg.get("default_fp_mid") is not None:
        lines.append(f'set(FP_M "{cfg["default_fp_mid"]}" CACHE STRING "mid precision (FP_M)")')
    lines += [
        f"",

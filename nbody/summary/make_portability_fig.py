#!/usr/bin/env python3
"""
nbody の性能可搬性 (performance portability) 図の生成器
  ...
  nbody は N を大きくすると GPU が飽和して実装間の差が見えなくなるため、
  飽和が始まる手前 (N < 1e5) に限定して選ぶ。
...
使い方 (このディレクトリで):
    ...
    python3 make_portability_fig.py --N 32768  # N を明示指定

    入力 : ./nbody.csv
    出力 : ./figs/nbody_portability.{png,pdf}
           ./figs/nbody_portability_best.csv  (棒の元になった best 値)
"""
...
APP = "nbody"
APP_TITLE = "N-body (direct)"
HERE = os.path.dirname(os.path.abspath(__file__))

# N の自動選択に設ける上限 (None なら制限なし)。
#   N>=1.3e5 で GPU が飽和し実装間の差が潰れるため上限を置く。
N_LIMIT = 100_000

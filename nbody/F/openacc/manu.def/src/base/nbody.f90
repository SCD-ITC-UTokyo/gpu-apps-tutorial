...
!$acc kernels present(ipos,iacc,jpos)
...
#ifdef CALCULATE_POTENTIAL
!$acc kernels present(acc,pos)
!$acc loop independent &
!$acc private(i,ai)
#else
!$acc kernels present(acc)
!$acc loop independent &
!$acc private(i,ai)
#endif
    do i=1, Ni
...
!$acc kernels present(vel_src,acc,vel)
!$acc loop independent &
!$acc private(i,vi,ai)
    do i=1, num
       !! initialization

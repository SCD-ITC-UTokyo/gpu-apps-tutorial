...
!$acc kernels present(ipos,iacc,jpos) vector_length(NTHREADS)
...
#ifdef CALCULATE_POTENTIAL
!$acc kernels present(acc,pos) vector_length(NTHREADS)
!$acc loop independent &
!$acc private(i,ai)
#else
!$acc kernels present(acc) vector_length(NTHREADS)
!$acc loop independent &
!$acc private(i,ai)
#endif
    do i=1, Ni
...
!$acc kernels present(vel,acc) vector_length(NTHREADS)
!$acc loop independent &
!$acc private(i,vi,ai)
    do i=1, num
       !! initialization
...
!$acc kernels present(pos,vel) vector_length(NTHREADS)
!$acc loop independent &
!$acc private(i,pi,vi)
    do i=1, num
       !! initialization
...
!$acc kernels present(vel_src,acc,vel) vector_length(NTHREADS)
!$acc loop independent &
!$acc private(i,vi,ai)
    do i=1, num
       !! initialization
...
!$acc enter data create(pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))

     !! generate initial-condition
     call init_set_uniform_sphere( num, pos, vel, M_tot, rad, virial, newton )
!$acc update device(pos(1:num), vel(1:num))
...
!$acc update host(pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))
     ...
     !! memory deallocation
!$acc exit data delete(pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))

...
!!!  - GPU Acceleration: Standard Parallel
!!!  - Memory Management: Auto(controlled by hardware or runtime)
!!!  - Optimization: -Mfprelaxed=rsqrt
...
!!!
  subroutine calc_acc( Ni, ipos, iacc, Nj, jpos, eps2 )
    ...
    integer :: i
#if defined(__NVCOMPILER) || defined(__INTEL_LLVM_COMPILER)
    integer :: j
    type(position) :: pi, pj
...
#if defined(__NVCOMPILER) || defined(__INTEL_LLVM_COMPILER)
    do concurrent(i=1:Ni) local(j,pi,pj,ai,dx,dy,dz,r2,r_inv,r2_inv,alp)
#else
...
#if defined(__NVCOMPILER) || defined(__INTEL_LLVM_COMPILER)
    do concurrent(i=1:num) local(vi,ai)
#else
...
#if defined(__NVCOMPILER) || defined(__INTEL_LLVM_COMPILER)    
    do concurrent(i=1:num) local(pi,vi)
#else
    do concurrent(i=1:num)
       block
       type(position) :: pi
...
!!!
  subroutine allocate_Nbody_particles( pos, vel, vel_tmp, acc, num )
    ...
    do concurrent(i=1:num)
       pos(i) = position( 0.0, 0.0, 0.0, 0.0 )
       vel(i) = velocity( 0.0, 0.0, 0.0, 0.0 )
       vel_tmp(i) = velocity( 0.0, 0.0, 0.0, 0.0 )
       acc(i) = acceleration( 0.0, 0.0, 0.0, 0.0 )
    end do

...
!!!
  subroutine calc_acc( Ni, ipos, iacc, Nj, jpos, eps2 )
    ...
    real(flt_acc) :: alp
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: ipos(1:Ni),iacc(1:Ni),jpos(1:Nj)) &
!$omp private(i,pi,ai,j,pj,dx,dy,dz,r2,r_inv,r2_inv,alp)
    do i=1, Ni
       !! initialization
       pi = ipos(i)
...
#ifdef CALCULATE_POTENTIAL
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: acc(1:Ni),pos(1:Ni)) &
!$omp private(i,ai)
#else
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: acc(1:Ni)) &
!$omp private(i,ai)
#endif
    do i=1, Ni
       !! initialization
...
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: vel(1:num),acc(1:num)) &
!$omp private(i,vi,ai)
    do i=1, num
       !! initialization
       vi = vel(i)
...
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: pos(1:num),vel(1:num)) &
!$omp private(i,pi,vi)
    do i=1, num
       !! initialization
       pi = pos(i)
...
!$omp target teams loop thread_limit(NTHREADS) &
!$omp map(alloc: vel_src(1:num),acc(1:num),vel(1:num)) &
!$omp private(i,vi,ai)
    do i=1, num
       !! initialization
       vi = vel_src(i)
...
program main
  ...
  do i=1, num_bin
     ...
     !$omp target enter data map(alloc : pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))

     !! generate initial-condition
     call init_set_uniform_sphere( num, pos, vel, M_tot, rad, virial, newton )
     !$omp target update to(pos(1:num), vel(1:num))
     ...
     !$omp target update from(pos(1:num), vel(1:num), acc(1:num))
...
!$omp target update from(pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))
     ...
     !! memory deallocation
     !$omp target exit data map(delete: pos(1:num), vel(1:num), vel_tmp(1:num), acc(1:num))

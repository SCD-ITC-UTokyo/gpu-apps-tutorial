...
///
static inline void write_snapshot(const type::int_idx num, type::position *__restrict pos, type::velocity *__restrict vel, type::acceleration *__restrict acc, const char *const filename, const int32_t snp_id, const type::fp_m time, conservatives &error) {
  // open the target file

  //---- modified by MIZUHO-RT to avoid an error when linking with libstdc++.
  ...
  /*
  char name[1024];
  snprintf( name, sizeof(name), "%s_snp%03d.h5", filename, snp_id );
  const auto hdf5_file = std::string(name) + ".h5";
  */
  //---- modified by MIZUHO-RT.
  ...
  //std::ofstream xml(folder_dat + std::string(name) + ".xdmf", std::ios::out);
  //---- modified by MIZUHO-RT.

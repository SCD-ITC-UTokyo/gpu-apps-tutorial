...
#pragma acc kernels present(ipos,iacc,jpos)
...
#ifdef CALCULATE_POTENTIAL
#pragma acc kernels present(acc,pos)
#pragma acc loop independent
#else
#pragma acc kernels present(acc)
#pragma acc loop independent
#endif
  for (std::remove_const_t<decltype(Ni)> i = 0; i < Ni; i++) {
    // initialization
...
#pragma acc kernels present(vel,acc)
#pragma acc loop independent
...
#pragma acc kernels present(pos,vel)
#pragma acc loop independent
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto pi = pos[i];
...
#pragma acc kernels present(vel_src,acc,vel)
#pragma acc loop independent
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto vi = vel_src[i];

...
#pragma acc kernels present(ipos,iacc,jpos) vector_length(NTHREADS)
...
#ifdef CALCULATE_POTENTIAL
#pragma acc kernels present(acc,pos) vector_length(NTHREADS)
#pragma acc loop independent
#else
#pragma acc kernels present(acc) vector_length(NTHREADS)
#pragma acc loop independent
#endif
  for (std::remove_const_t<decltype(Ni)> i = 0; i < Ni; i++) {
    // initialization
...
#pragma acc kernels present(vel,acc) vector_length(NTHREADS)
#pragma acc loop independent
...
#pragma acc kernels present(pos,vel) vector_length(NTHREADS)
#pragma acc loop independent
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto pi = pos[i];
...
#pragma acc kernels present(vel_src,acc,vel) vector_length(NTHREADS)
#pragma acc loop independent
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto vi = vel_src[i];
...
#pragma acc update host(pos[0:num], vel[0:num], acc[0:num])
...
#pragma acc update host(pos[0:num], vel[0:num], vel_tmp[0:num], acc[0:num])

...
if [ "$task" = "build" ]; then
    if [ -n "${PJM_O_WORKDIR}" ]; then
	echo "Please compile this program in frontend."
	exit 1
    fi
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=32   clean install
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=64   clean install
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=128  clean install
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=256  clean install
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=512  clean install
#    make -C src -f Makefile.gpu FP_L=32 FP_M=32 BENCH=1 NTHREADS=1024 clean install
...
if [ "$task" = "optim" ]; then
    rm -f log/optim.gpu_run.csv
    for t in 32 64 128 256 512 1024; do
	for n in $(seq 1 11) ; do
	    bin/nbody.bench.gpu.32.32.$t --file="optim.gpu" \

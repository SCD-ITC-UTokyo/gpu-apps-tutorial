...
#pragma omp target teams loop map(alloc: ipos[0:Ni], jpos[0:Nj], iacc[0:Ni]) thread_limit(NTHREADS)
...
#ifdef CALCULATE_POTENTIAL
#pragma omp target teams loop map(alloc: acc[0:Ni], pos[0:Ni]) thread_limit(NTHREADS)
#else
#pragma omp target teams loop map(alloc: acc[0:Ni]) thread_limit(NTHREADS)
#endif
  for (std::remove_const_t<decltype(Ni)> i = 0; i < Ni; i++) {
    // initialization
    auto ai = acc[i];
...
#pragma omp target teams loop map(alloc: acc[0:num], vel[0:num]) thread_limit(NTHREADS)
...
#pragma omp target teams loop map(alloc: pos[0:num], vel[0:num]) thread_limit(NTHREADS)
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto pi = pos[i];
    const auto vi = vel[i];
...
#pragma omp target teams loop map(alloc: vel_src[0:num], acc[0:num], vel[0:num]) thread_limit(NTHREADS)
  for (std::remove_const_t<decltype(num)> i = 0; i < num; i++) {
    // initialization
    auto vi = vel_src[i];
    const auto ai = acc[i];

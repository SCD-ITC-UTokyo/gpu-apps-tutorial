...
///
/// @brief Kokkos::View設定 (Unified - Kokkos::CudaUVMSpace>)
///
...
int main(int argc, char** argv) {
  ...
  for (std::remove_const_t<decltype(num_bin)> i = 0; i < num_bin; i++) {
    ...
    io::write_snapshot(num, pos.data(), vel.data(), acc.data(), file.c_str(), present, time, error);
    ...
    while (present < snp_fin) {
      ...
      if (present > previous) {
        ...
        io::write_snapshot(num, pos.data(), vel_tmp.data(), acc.data(), file.c_str(), present, time, error);
      }
    }
#else   // BENCHMARK_MODE
  // launch benchmark

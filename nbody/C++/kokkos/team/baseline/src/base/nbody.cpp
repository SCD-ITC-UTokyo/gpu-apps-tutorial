...
static inline void kick(const type::int_idx num, KVvel1D vel, KVacc1D acc, const type::flt_vel dt) {
  ...
  using member_type = team_policy::member_type;
  const int chunk = NCHUNK;  // 1チームあたりの要素数
  const int league_size = (num + chunk - 1) / chunk;
  team_policy policy(league_size, Kokkos::AUTO());
  Kokkos::parallel_for(
    "kick", policy,
    KOKKOS_LAMBDA(const member_type& team) {
      // 各チームが処理する範囲(begin～end)を計算
      const int begin = team.league_rank() * chunk;
      const int end   = (begin + chunk < num) ? (begin + chunk) : num;
...
static inline void allocate_Nbody_particles(KVpos1D pos, KVvel1D vel, KVvel1D vel_tmp, KVacc1D acc, const type::int_idx num) {
  ...
  using member_type = team_policy::member_type;
  const int chunk = NCHUNK;  // 1チームあたりの要素数
  const int league_size = (size + chunk - 1) / chunk;
  team_policy policy(league_size, Kokkos::AUTO());
  Kokkos::parallel_for(
    "allocate_Nbody_particles", policy,
    KOKKOS_LAMBDA(const member_type& team) {
      // 各チームが処理する範囲(begin～end)を計算
      const int begin = team.league_rank() * chunk;

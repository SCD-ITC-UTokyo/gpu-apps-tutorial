...
int main(int argc, char *argv[])
{
    ...
    const int nt = 100000;
#else
    const int nt = (int)(512.0 * 256.0 * 256.0 / (double)nx / (double)nx);
#endif
    double time = 0.0;
    int    icnt = 0;
    double flop = 0.0;
    ...
    Kokkos::View<float*, Kokkos::CudaUVMSpace> f("f", ln);
    Kokkos::View<float*, Kokkos::CudaUVMSpace> fn("fn", ln);
    ...
    const double faccuracy = sqrt(ferr / (double)N);

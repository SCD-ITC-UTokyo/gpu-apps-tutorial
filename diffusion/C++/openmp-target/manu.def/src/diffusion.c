...
#pragma omp target teams loop collapse(3) map(alloc: f[0:ln], fn[0:ln])
    for(int k = 0; k < nz; k++) {
        for (int j = 0; j < ny; j++) {
            for (int i = 0; i < nx; i++) {
...
#pragma omp target teams loop collapse(3) map(alloc: f[0:ln])
...
#pragma omp target teams loop collapse(3) map(alloc: f[0:ln]) reduction(+:ferr)
    for(int k = 0; k < nz; k++) {
        for(int j = 0; j < ny; j++) {
            for(int i = 0; i < nx; i++) {
                const int ix = nx * ny * (k + mgn) + nx * j + i;

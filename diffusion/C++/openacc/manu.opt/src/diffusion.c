...
#pragma acc kernels present(f,fn) vector_length(NTHREADS)
...
#pragma acc kernels present(f) vector_length(NTHREADS)
#pragma acc loop independent collapse(3)
...
#pragma acc kernels present(f) vector_length(NTHREADS)
#pragma acc loop independent reduction(+:ferr) collapse(3)
    for(int k = 0; k < nz; k++) {
        for(int j = 0; j < ny; j++) {
            for(int i = 0; i < nx; i++) {
                const int ix = nx * ny * (k + mgn) + nx * j + i;

#!/usr/bin/env python3
"""
diffusion.csv インタラクティブ可視化ダッシュボード生成器

使い方:
    python3 visualize.py [csv_path] [out_html]

    csv_path: default = ./diffusion.csv (同一ディレクトリ)
    out_html: default = ./diffusion_dashboard.html (同一ディレクトリ)
...
CSV_PATH = sys.argv[1] if len(sys.argv) > 1 else os.path.join(ROOT, "diffusion.csv")
OUT_HTML = sys.argv[2] if len(sys.argv) > 2 else os.path.join(ROOT, "diffusion_dashboard.html")
...
<head>
<meta charset="UTF-8">
<title>diffusion benchmark dashboard</title>

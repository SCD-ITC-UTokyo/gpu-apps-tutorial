#!/usr/bin/env python3
"""
results/<machine>/*.log を diffusion.csv 形式 (21 カラム) に集約変換する。
...
バイナリ自身は 10 カラム CSV を出力 (`# binary,N(=nx*ny*nz),time_sec,...`):
    ...
    ./build-local-gpu/diffusion.gpu.32,262144,6.685e-02,4.176e+01,1.214e-05,...
...
使い方:
    ...
    # 既存の diffusion.csv に追記
    python3 parse_logs.py --input ../results/local --append diffusion.csv
...
# 例: .../C++/openmp-target/auto.def/build-local-gpu/diffusion.gpu.32
PATH_RE = re.compile(
...
def parse_log_file(log_file: Path) -> list[dict]:
    ...
    with open(log_file) as f:
        for line in f:
            ...
            # fields[8] = num_time_steps_logged, [9] = last_sim_time は diffusion.csv 21 カラムに含まれない
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="results/<machine>/*.log を diffusion.csv 形式 (21 列) に集約変換")
    ...
    ap.add_argument("--append", default=None, metavar="EXISTING_CSV",
                    help="既存 CSV (例: summary/diffusion.csv) に追記 (header はスキップ)")
    args = ap.parse_args()

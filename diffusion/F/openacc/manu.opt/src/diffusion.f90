...
contains
  ...
  real(dp) function diffusion3d(nx, ny, nz, dx, dy, dz, dt, kappa, f, fn)
    ...
    !$acc kernels present(f, fn) vector_length(NTHREADS)
    !$acc loop independent collapse(3) private(i,j,k,w,e,n,s,b,t)
    do k = 1, nz
       do j = 1, ny
          do i = 1, nx
  ...
  subroutine init(nx, ny, nz, dx, dy, dz, f)
    ...
    !$acc kernels present(f) vector_length(NTHREADS)
    !$acc loop independent collapse(3) private(i,j,k,x,y,z)
    do k = 0, nz+1
       do j = 1, ny
          do i = 1, nx
  ...
  real(dp) function err(time, nx, ny, nz, dx, dy, dz, kappa, f)
    ...
    real(fp),intent(in),dimension(1:nx,1:ny,0:nz+1) :: f
    real(fp) :: kx,ky,kz,ax,ay,az,x,y,z,pi,f0
    real(dp) :: ferr
    integer :: i,j,k
    ...
    !$acc kernels present(f) vector_length(NTHREADS)
    !$acc loop independent collapse(3) private(i,j,k,x,y,z) reduction(+:ferr)
    do k = 1, nz
       do j = 1, ny
          do i = 1, nx

...
contains
  ...
  real(dp) function diffusion3d(nx, ny, nz, dx, dy, dz, dt, kappa, f, fn)
    ...
    !$omp target teams loop collapse(3) private(i,j,k,w,e,n,s,b,t) thread_limit(NTHREADS)
    do k = 1, nz
       do j = 1, ny
          do i = 1, nx
  ...
  subroutine init(nx, ny, nz, dx, dy, dz, f)
    ...
    !$omp target teams loop collapse(3) private(i,j,k,x,y,z) thread_limit(NTHREADS)
    do k = 0, nz+1
       do j = 1, ny
          do i = 1, nx
  ...
  real(dp) function err(time, nx, ny, nz, dx, dy, dz, kappa, f)
    ...
    !$omp target teams loop collapse(3) reduction(+:ferr) private(i,j,k,x,y,z,f0) thread_limit(NTHREADS)
    do k = 1, nz
       do j = 1, ny
          do i = 1, nx

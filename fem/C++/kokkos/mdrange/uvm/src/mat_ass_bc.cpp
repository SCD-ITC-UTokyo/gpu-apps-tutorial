...
**/
  ...
  Kokkos::parallel_for("mat_ass_bc_loop1", policy_1d(NODGRP_INDEX[ib0],NODGRP_INDEX[ib0+1]), KOKKOS_LAMBDA(const int ib){
    int in=NODGRP_ITEM_d(ib);
    IWKX_d(in-1)=1;
  });
  Kokkos::fence();

  Kokkos::parallel_for("mat_ass_bc_loop2", policy_1d(0,NP), KOKKOS_LAMBDA(const int in){
    if( IWKX_d(in) == 1 ){
      B_d(in)= 0.e0;
      D_d(in)= 1.e0;
  ...
  Kokkos::parallel_for("mat_ass_bc_loop3", policy_1d(0,NP), KOKKOS_LAMBDA(const int in){
    for(int k=indexLU_d(in);k<indexLU_d(in+1);k++){

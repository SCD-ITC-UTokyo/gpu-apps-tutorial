...
void MAT_ASS_MAIN()
{
  ...
  WEI_d(0)= 1.0000000000e0;
  WEI_d(1)= 1.0000000000e0;
...
***/
  ...
  for(icol=1;icol<NP;icol++) {
    ...
    if (IWKX[icel][1]== 0){
      in1=ICELNOD_g(icel,0)-1;
      in2=ICELNOD_g(icel,1)-1;
      in3=ICELNOD_g(icel,2)-1;
      in4=ICELNOD_g(icel,3)-1;
      in5=ICELNOD_g(icel,4)-1;
      in6=ICELNOD_g(icel,5)-1;
      in7=ICELNOD_g(icel,6)-1;
      in8=ICELNOD_g(icel,7)-1;
...
***/
  ...
  for(ip=0;ip<2;ip++){
	...
	TP1= 1.e0 + POS[kp];
	TM1= 1.e0 - POS[kp];
	SHAPE_d(ip,jp,kp,0)= O8th * QM1 * EM1 * TM1;
	SHAPE_d(ip,jp,kp,1)= O8th * QP1 * EM1 * TM1;
	SHAPE_d(ip,jp,kp,2)= O8th * QP1 * EP1 * TM1;
	SHAPE_d(ip,jp,kp,3)= O8th * QM1 * EP1 * TM1;
	SHAPE_d(ip,jp,kp,4)= O8th * QM1 * EM1 * TP1;
	SHAPE_d(ip,jp,kp,5)= O8th * QP1 * EM1 * TP1;
	SHAPE_d(ip,jp,kp,6)= O8th * QP1 * EP1 * TP1;
	SHAPE_d(ip,jp,kp,7)= O8th * QM1 * EP1 * TP1;
	PNQ_d(jp,kp,0)= - O8th * EM1 * TM1;
	PNQ_d(jp,kp,1)= + O8th * EM1 * TM1;
	PNQ_d(jp,kp,2)= + O8th * EP1 * TM1;
	PNQ_d(jp,kp,3)= - O8th * EP1 * TM1;
	PNQ_d(jp,kp,4)= - O8th * EM1 * TP1;
	PNQ_d(jp,kp,5)= + O8th * EM1 * TP1;
	PNQ_d(jp,kp,6)= + O8th * EP1 * TP1;
	PNQ_d(jp,kp,7)= - O8th * EP1 * TP1;
	PNE_d(ip,kp,0)= - O8th * QM1 * TM1;
	PNE_d(ip,kp,1)= - O8th * QP1 * TM1;
	PNE_d(ip,kp,2)= + O8th * QP1 * TM1;
	PNE_d(ip,kp,3)= + O8th * QM1 * TM1;
	PNE_d(ip,kp,4)= - O8th * QM1 * TP1;
	PNE_d(ip,kp,5)= - O8th * QP1 * TP1;
	PNE_d(ip,kp,6)= + O8th * QP1 * TP1;
	PNE_d(ip,kp,7)= + O8th * QM1 * TP1;
	PNT_d(ip,jp,0)= - O8th * QM1 * EM1;
	PNT_d(ip,jp,1)= - O8th * QP1 * EM1;
	PNT_d(ip,jp,2)= - O8th * QP1 * EP1;
	PNT_d(ip,jp,3)= - O8th * QM1 * EP1;
	PNT_d(ip,jp,4)= + O8th * QM1 * EM1;
	PNT_d(ip,jp,5)= + O8th * QP1 * EM1;
	PNT_d(ip,jp,6)= + O8th * QP1 * EP1;
	PNT_d(ip,jp,7)= + O8th * QM1 * EP1;
      }
    }
  }
  ...
  Kokkos::parallel_for("mat_ass_main", policy_1d(ELMCOLORindex[icol-1],ELMCOLORindex[icol],Kokkos::ChunkSize(CHUNK_SIZE_ASS)), KOKKOS_LAMBDA(const int icel0){

...
**/
  ...
  Kokkos::parallel_for("init_WW", N, KOKKOS_LAMBDA(const int i)
  {
    for(int j=0;j<4;j++){
      WW(j, i) = 0.0;
    }
...
/**
   ...
   +-----------------------+
**/
  Kokkos::parallel_for("r0", N, KOKKOS_LAMBDA(const int j)
  {
    WW(DD, j)= 1.0/D(j);
  ...
  BNRM2= 0.e0;
  Kokkos::parallel_reduce("BNRM2", N, KOKKOS_LAMBDA(const int i, KREAL &BNRM2_l)
  {
    BNRM2_l+= B(i)*B(i);
  }, BNRM2);
  Kokkos::fence();
...
**/
{
  Kokkos::parallel_for("z", N, KOKKOS_LAMBDA(const int i)
  {
...
**/
  if( ITER == 1 ){
    Kokkos::parallel_for("p_iter1", N, KOKKOS_LAMBDA(const int i)
    {
      WW(P, i)=WW(Z, i);
    });
  }else{
    BETA= RHO / RHO1;
    Kokkos::parallel_for("p_other", N, KOKKOS_LAMBDA(const int i)
    {
...
**/
  C1= 0.e0;
  Kokkos::parallel_reduce("C1", N, KOKKOS_LAMBDA(const int i, KREAL &C1_l)
  {
    C1_l+=WW(P, i)*WW(Q, i);
  }, C1);
  Kokkos::fence();
...
/**
   ...
   +----------------------+
**/
  Kokkos::parallel_for("xr", N, KOKKOS_LAMBDA(const int i)
  {
    X (i)   +=  ALPHA *WW(P, i);
    WW(R, i)+= -ALPHA *WW(Q, i);
  });
  ...
  DNRM2= 0.e0;
  Kokkos::parallel_reduce("DNRM2", N, KOKKOS_LAMBDA(const int i, KREAL &DNRM2_l)
  {

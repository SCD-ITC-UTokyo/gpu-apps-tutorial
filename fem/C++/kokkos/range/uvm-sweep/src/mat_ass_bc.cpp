...
**/
  ...
  const int inum=NODGRP_INDEX[ib0+1]-NODGRP_INDEX[ib0];
  const int ioff=NODGRP_INDEX[ib0];
  Kokkos::parallel_for("mat_ass_bc_loop1", inum, KOKKOS_LAMBDA(const int ib){
    int in=NODGRP_ITEM_d(ib+ioff);
    IWKX_d(in-1)=1;
  });
  Kokkos::fence();

  Kokkos::parallel_for("mat_ass_bc_loop2", NP, KOKKOS_LAMBDA(const int in){
    if( IWKX_d(in) == 1 ){
      B_d(in)= 0.e0;
      D_d(in)= 1.e0;
  ...
  Kokkos::parallel_for("mat_ass_bc_loop3", NP, KOKKOS_LAMBDA(const int in){
    for(int k=indexLU_d(in);k<indexLU_d(in+1);k++){

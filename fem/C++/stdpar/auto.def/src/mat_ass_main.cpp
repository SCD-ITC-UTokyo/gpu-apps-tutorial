...
***/
  ...
  for( icol=1; icol< ELMCOLORtot+1; icol++){
      ...
      [&](int icel0)
    {
      ...
      int in3=ICELNOD[icel*8+2];
      int in4=ICELNOD[icel*8+3];
      int in5=ICELNOD[icel*8+4];
      int in6=ICELNOD[icel*8+5];
      int in7=ICELNOD[icel*8+6];
      int in8=ICELNOD[icel*8+7];
      /**
       **
       ** JACOBIAN & INVERSE JACOBIAN
       **/
      KREAL X1=XYZ[(in1-1)*3+0];
      KREAL X2=XYZ[(in2-1)*3+0];
      KREAL X3=XYZ[(in3-1)*3+0];
      KREAL X4=XYZ[(in4-1)*3+0];
      KREAL X5=XYZ[(in5-1)*3+0];
      KREAL X6=XYZ[(in6-1)*3+0];
      KREAL X7=XYZ[(in7-1)*3+0];
      KREAL X8=XYZ[(in8-1)*3+0];
    
      KREAL Y1=XYZ[(in1-1)*3+1];
      KREAL Y2=XYZ[(in2-1)*3+1];
      KREAL Y3=XYZ[(in3-1)*3+1];
      KREAL Y4=XYZ[(in4-1)*3+1];
      KREAL Y5=XYZ[(in5-1)*3+1];
      KREAL Y6=XYZ[(in6-1)*3+1];
      KREAL Y7=XYZ[(in7-1)*3+1];
      KREAL Y8=XYZ[(in8-1)*3+1];
    
      KREAL Z1=XYZ[(in1-1)*3+2];
      KREAL Z2=XYZ[(in2-1)*3+2];
      KREAL Z3=XYZ[(in3-1)*3+2];
      KREAL Z4=XYZ[(in4-1)*3+2];
      KREAL Z5=XYZ[(in5-1)*3+2];
      KREAL Z6=XYZ[(in6-1)*3+2];
      KREAL Z7=XYZ[(in7-1)*3+2];
      KREAL Z8=XYZ[(in8-1)*3+2];
    
      KREAL QVC= O8th*(X1+X2+X3+X4+X5+X6+X7+X8+Y1+Y2+Y3+Y4+Y5+Y6+Y7+Y8);
	...
	for(int je=0;je<8;je++){
	  ...
	  int kk=-1;
	  if( jp != ip ){
	    int iiS=indexLU[ip-1];
	    int iiE=indexLU[ip  ];
	    for( int k=iiS;k<iiE;k++){
	      if( itemLU[k] == jp-1 ){
		kk=k;
		break;
	      }
	  ...
	  for(int kpn=0;kpn<2;kpn++){
		...
		KREAL coef= WEI[ipn]*WEI[jpn]*WEI[kpn];
	      
		KREAL PNXi= PNX[ipn][jpn][kpn][ie];
		KREAL PNYi= PNY[ipn][jpn][kpn][ie];
		KREAL PNZi= PNZ[ipn][jpn][kpn][ie];
	      
		KREAL PNXj= PNX[ipn][jpn][kpn][je];
		KREAL PNYj= PNY[ipn][jpn][kpn][je];
		KREAL PNZj= PNZ[ipn][jpn][kpn][je];
		...
		KREAL SHi= SHAPE[ipn][jpn][kpn][ie];
		QV0+= SHi * QVOL * coef *
#if   FP ==  32
		  fabsf(DETJ[ipn][jpn][kpn]);
#elif FP ==  64

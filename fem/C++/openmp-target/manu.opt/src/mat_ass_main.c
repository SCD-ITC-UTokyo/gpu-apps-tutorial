...
#pragma omp target enter data				\
  map(alloc: AMAT[0:NPLU],B[0:NP],D[0:NP],X[0:NP])
...
#pragma omp target teams distribute parallel for \
  map(alloc: B[0:NP],D[0:NP],X[0:NP]) \
  private(i) \
  thread_limit(NTHREADS)
  for(i=0;i<NP;i++){
    B[i]=0.0;
...
#pragma omp target data \
  map(to: ELMCOLORindex[0:NP+1],ELMCOLORitem[0:ICELTOT]) \
  map(to: ICELNOD[0:ICELTOT*8],XYZ[0:NP*3])		 \
  map(to: PNQ[0:2*2*8],PNE[0:2*2*8],PNT[0:2*2*8])	 \
  map(to: indexLU[0:NP+1],itemLU[0:NPLU])		 \
  map(to: WEI[0:2],SHAPE[0:2*2*2*8])
  {
  
  for( icol=1; icol< ELMCOLORtot+1; icol++){
#pragma omp target teams distribute parallel for	       \
  map(alloc: ELMCOLORindex[0:NP+1],ELMCOLORitem[0:ICELTOT])  \
  map(alloc: ICELNOD[0:ICELTOT*8],XYZ[0:NP*3])	       \
  map(alloc: PNQ[0:2*2*8],PNE[0:2*2*8],PNT[0:2*2*8])	       \
  map(alloc: indexLU[0:NP+1],itemLU[0:NPLU])		       \
  map(alloc: WEI[0:2],SHAPE[0:2*2*2*8])		       \
  map(alloc: AMAT[0:NPLU],B[0:NP],D[0:NP],X[0:NP])	       \
  private(icel0,icel)					       \
  ...
  thread_limit(NTHREADS)
    ...
    // pragma omp target end parallel for
  } // icol
...
#pragma omp target exit data \
  map(delete: AMAT[0:NPLU],B[0:NP],D[0:NP],X[0:NP])
  free(AMAT);
  free(B);
  free(D);
  free(X);

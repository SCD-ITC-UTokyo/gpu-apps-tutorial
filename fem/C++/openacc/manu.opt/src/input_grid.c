  ...
  for(k=0;k<nxe;k++){
    for(j=0;j<nxe;j++){
      for(i=0;i<nxe;i++){
        ...
        ICELNOD[icel*8+0]= n0;
        ICELNOD[icel*8+1]= n0 + 1;
        ICELNOD[icel*8+2]= n0 + 1 + nxn;
        ICELNOD[icel*8+3]= n0     + nxn;
        ICELNOD[icel*8+4]= n0          + nn;
        ICELNOD[icel*8+5]= n0 + 1      + nn;
        ICELNOD[icel*8+6]= n0 + 1 + nxn+ nn;
        ICELNOD[icel*8+7]= n0     + nxn+ nn;
        icel++;
      }
    }
  }
...
**/
  ...
  NODGRP_ITEM=(KINT*)malloc(sizeof(KINT)*NODGRP_INDEX[NODGRPtot]);

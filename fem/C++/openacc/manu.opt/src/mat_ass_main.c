...
#pragma acc enter data \
  create(AMAT[0:NPLU],B[0:NP],D[0:NP],X[0:NP])
...
#pragma acc data					\
  copyin(ELMCOLORindex[0:NP+1],ELMCOLORitem[0:ICELTOT]) \
  copyin(ICELNOD[0:ICELTOT*8],XYZ[0:NP*3])		\
  copyin(PNQ[0:2*2*8],PNE[0:2*2*8],PNT[0:2*2*8])	\
  copyin(indexLU[0:NP+1],itemLU[0:NPLU])		\
  copyin(WEI[0:2],SHAPE[0:2*2*2*8])
  {
  
  for( icol=1; icol< ELMCOLORtot+1; icol++){
#pragma acc parallel loop		     \
  present(ELMCOLORindex,ELMCOLORitem)	     \
  present(ICELNOD,XYZ)			     \
  present(PNQ,PNE,PNT)			     \
  present(indexLU,itemLU)		     \
  present(WEI,SHAPE)			     \
  present(AMAT,B,D,X)			     \
  private(icel0,icel)			     \
...
void MAT_ASS_CLEAR()
{
#pragma acc exit data delete(AMAT,B,D,X)
  free(AMAT);
  free(B);
  free(D);
  free(X);

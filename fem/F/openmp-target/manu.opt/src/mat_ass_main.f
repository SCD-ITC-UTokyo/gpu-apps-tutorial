...
!$omp& map(alloc: AMAT(1:NPLU),B(1:NP),D(1:NP),X(1:NP))
...
!$omp target teams distribute parallel do
!$omp& map(alloc: B(1:NP),D(1:NP),X(1:NP))
!$omp& private(i)
!$omp& thread_limit(NTHREADS)
      do i=1, NP
         B(i)=0.d0
...
!$omp target data
!$omp& map(to: ELMCOLORindex(0:NP),ELMCOLORitem(1:ICELTOT))
!$omp& map(to: ICELNOD(1:ICELTOT,1:8),XYZ(1:NP,1:3))
!$omp& map(to: PNQ(1:2,1:2,1:8),PNE(1:2,1:2,1:8),PNT(1:2,1:2,1:8))
!$omp& map(to: indexLU(0:NP),itemLU(1:NPLU))
!$omp& map(to: WEI(1:2),SHAPE(1:2,1:2,1:2,1:8))
      
      do icol= 1, ELMCOLORtot
!$omp target teams distribute parallel do
!$omp& map(alloc: ELMCOLORindex(0:NP),ELMCOLORitem(1:ICELTOT))
!$omp& map(alloc: ICELNOD(1:ICELTOT,1:8),XYZ(1:NP,1:3))
!$omp& map(alloc: PNQ(1:2,1:2,1:8),PNE(1:2,1:2,1:8),PNT(1:2,1:2,1:8))
!$omp& map(alloc: indexLU(0:NP),itemLU(1:NPLU))
!$omp& map(alloc: WEI(1:2),SHAPE(1:2,1:2,1:2,1:8))
!$omp& map(alloc: AMAT(1:NPLU),B(1:NP),D(1:NP),X(1:NP))
!$omp& private(icel0,icel)
...
!$omp target exit data map(delete: AMAT(1:NPLU),B(1:NP),D(1:NP),X(1:NP))

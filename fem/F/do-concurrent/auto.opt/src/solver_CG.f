...
!C===
      ...
      do concurrent (i=1:NP) local(j,WVAL)
c$$$    WW(i,DD)= 1.d0/D(i)
        DW(i)= 1.d0/D(i)
        WVAL= B(i) - D(i)*X(i)
...
c$$$    WW(i,R)= WVAL
      ...
      BNRM2= 0.d0
      do concurrent (i=1:NP) reduce(+:BNRM2)
        BNRM2= BNRM2 + B(i)**2
      enddo
      ITER = 0
!C===
...
!C===
      RHO= 0.d0
      do concurrent (i=1:NP) reduce(+:RHO)
...
!C===
      C1= 0.d0
      do concurrent (i=1:NP) reduce(+:C1)
...
c$$$    WW(i,R)= WW(i,R) - ALPHA * WW(i,Q)
      ...
      DNRM2= 0.d0
      do concurrent (i=1:NP) reduce(+:DNRM2)

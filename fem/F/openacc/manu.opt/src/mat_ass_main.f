...
!$acc enter data
!$acc& create(AMAT(1:NPLU),B(1:NP),D(1:NP),X(1:NP))
...
!$acc data
!$acc& copyin(ELMCOLORindex(0:NP),ELMCOLORitem(1:ICELTOT))
!$acc& copyin(ICELNOD(1:ICELTOT,1:8),XYZ(1:NP,1:3))
!$acc& copyin(PNQ(1:2,1:2,1:8),PNE(1:2,1:2,1:8),PNT(1:2,1:2,1:8))
!$acc& copyin(indexLU(0:NP),itemLU(1:NPLU))
!$acc& copyin(WEI(1:2),SHAPE(1:2,1:2,1:2,1:8))
      
      do icol= 1, ELMCOLORtot
!$acc parallel loop
!$acc& present(ELMCOLORindex,ELMCOLORitem)
!$acc& present(ICELNOD,XYZ)
!$acc& present(PNQ,PNE,PNT)
!$acc& present(indexLU,itemLU)
!$acc& present(WEI,SHAPE)
!$acc& present(AMAT,B,D,X)
!$acc& private(icel0,icel)
!$acc& private(in1,in2,in3,in4,in5,in6,in7,in8)
!$acc& private(X1,X2,X3,X4,X5,X6,X7,X8)
!$acc& private(Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8)
!$acc& private(Z1,Z2,Z3,Z4,Z5,Z6,Z7,Z8)
!$acc& private(QVC,DETJ,PNX,PNY,PNZ)
!$acc& private(nodLOCAL)
!$acc& private(ie,je,ip,jp,kk,iiS,iiE,k)
!$acc& private(QV0,COEFij,ipn,jpn,kpn,coef)
!$acc& private(PNXi,PNYi,PNZi,PNXj,PNYj,PNZj,SHi)
!$acc& vector_length(NTHREADS)
...
!$acc exit data delete(AMAT,B,D,X)
